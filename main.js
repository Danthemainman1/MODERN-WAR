...
function performSimulationTick() {
    ...
    // Optimization: run global occupation fade less frequently with an equivalent decay curve.
    if (CONFIG.OCCUPATION_FADE && (simFrameCount % 3 === 0)) {
        const decay = Math.pow(0.98, 3);
        for (let i = 0; i < occupationMap.length; i++) {
            if (occupationMap[i] !== 0) {
                occupationMap[i] *= decay;
                if (Math.abs(occupationMap[i]) < 0.001) occupationMap[i] = 0;
            }
        }
    }

    // Optimization: avoid rescanning the entire map every tick for global metrics.
    ...
    let p1LandScore = cachedP1LandScore;
    if (shouldCountLand || simFrameCount < 30 || simFrameCount % 120 === 0) {
        let p1T = 0, p2T = 0;
        for (let i = 0; i < occupationMap.length; i++) {
            if (landMask[i] === 2) {
        ...
        cachedP1LandScore = (p1T + p2T > 0) ? (p1T / (p1T + p2T)) * 100 : 50;
        p1LandScore = cachedP1LandScore;
    }
...
function updateLoop(now) {
    ...
    // Optimization: recalculate heavy UI counters every other rendered frame.
    if (simFrameCount < 10 || simFrameCount % 2 === 0) {
